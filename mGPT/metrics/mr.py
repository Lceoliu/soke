...
class MRMetrics(Metric):
    ...
    def update(self, 
        ...
        # Keep metric math on float32 CPU tensors.
        # AMP test/eval can produce float16 tensors, but SVD in rigid alignment
        # does not support half dtype on CPU.
        joints_rst = joints_rst.detach().float().cpu()
        joints_ref = joints_ref.detach().float().cpu()
        vertices_rst = vertices_rst.detach().float().cpu()
        vertices_ref = vertices_ref.detach().float().cpu()
