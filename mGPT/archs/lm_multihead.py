...
class LMMultiHead(nn.Module):
    ...
    def forward_t5(
        ...
        # Decode
        # Only pass input_ids if inputs_embeds is None (cannot pass both)
        decoder_outputs = decoder(
            input_ids=decoder_input_ids if decoder_inputs_embeds is None else None,
        ...
        decoder_outputs_hand = decoder(
            input_ids=decoder_input_ids_hand if decoder_inputs_embeds is None else None,
            attention_mask=decoder_attention_mask,
            inputs_embeds=decoder_inputs_embeds,
            past_key_values=past_key_values,
            encoder_hidden_states=hidden_states,
        ...
        if self.num_heads > 2:
            decoder_outputs_rhand = decoder(
                input_ids=decoder_input_ids_rhand if decoder_inputs_embeds is None else None,
                attention_mask=decoder_attention_mask,
                inputs_embeds=decoder_inputs_embeds,
                past_key_values=past_key_values,
                encoder_hidden_states=hidden_states,
    ...
    def inference_decoder(
        ...
        elif self.model_type in ['mbart']:
            # For baseline mbart without multi-head, use only body decoder_input_ids
            decoder_inputs_embeds = token_embeds(decoder_input_ids)
        ...
        with torch.no_grad():
            ...
            for i in range(max_length):
                ...
                if do_sample:
                    ...
                    # Update finished flag for all model types (including mbart baseline)
