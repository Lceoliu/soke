...
# Qwen2.5-3B downstream pipeline:
# 1) optional motion-token cache generation
# 2) Qwen LM training
# 3) optional m2t evaluation (BLEU/ROUGE)
# 4) optional t2m sample generation + mesh visualization

CFG=${CFG:-"configs/soke.yaml"}
GPU_IDS=${GPU_IDS:-"0,1,2,3,4,5,6,7"}
NUM_NODES=${NUM_NODES:-1}
EXP_NAME=${EXP_NAME:-"SOKE_QWEN_LM"}
...
if "$QWEN_MODEL_PATH":
    cfg.model.params.lm.params.model_path = "$QWEN_MODEL_PATH"
if "$USE_LORA":
    cfg.model.params.lm.params.use_lora = str("$USE_LORA").lower() in ["1", "true", "yes", "y"]
if "$LORA_RANK":
    cfg.model.params.lm.params.lora_rank = int("$LORA_RANK")
if "$LORA_ALPHA":
    cfg.model.params.lm.params.lora_alpha = int("$LORA_ALPHA")
if "$LORA_DROPOUT":
    cfg.model.params.lm.params.lora_dropout = float("$LORA_DROPOUT")
if "$GRADIENT_CHECKPOINTING":
    cfg.model.params.lm.params.gradient_checkpointing = str("$GRADIENT_CHECKPOINTING").lower() in ["1", "true", "yes", "y"]
if "$GEN_MAX_NEW_TOKENS":
    cfg.model.params.lm.params.generation_max_new_tokens = int("$GEN_MAX_NEW_TOKENS")
if "$MAX_LENGTH":
...
if [[ "$TRAIN_LM" == "1" ]]; then
  ...
  trap - INT TERM EXIT
  if [[ $RC -ne 0 ]]; then
    echo "[FATAL] Qwen training failed, exit=$RC"
    exit "$RC"
  fi
fi
...
if [[ "$AUTO_EVAL_BLEU" == "1" ]]; then
  ...
  BLEU_CFG="/tmp/soke_qwen_bleu_eval_${TS}.yaml"
  "$PYTHON_BIN" - <<PY
from omegaconf import OmegaConf
cfg = OmegaConf.load("$RUN_CFG")
