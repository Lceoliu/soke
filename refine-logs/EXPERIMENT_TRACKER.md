...
| R023 | G1-a | G1 | Codebook utilization: Conv1d vs ST-GCN | Token entropy, utilization rate, dead codes on CSL train (500 samples) | CSL-Daily | train | entropy, util%, dead codes | MUST | DONE ⚠️ | Both VAEs: 100% utilization, entropy ≥90% of max, 0 dead codes. ⚠️ 测量离散LFQ token空间；mT5使用encode_continuous()，结论对mT5无关；对Qwen路径仍有效。Full report: experiments/analysis/g1_codebook_utilization/ |
| R024 | G1-c | G1 | Oracle NN decoder: upper-bound BLEU on discrete tokens | Nearest-neighbor retrieval by LFQ token histogram; measures LFQ token ceiling | CSL-Daily | train→test | oracle BLEU4 | MUST | DONE ⚠️ | conv1d oracle BLEU4=0.195, stgcn=0.121. ⚠️ 前提错误：mT5使用encode_continuous()而非LFQ token，本结果不是mT5输入空间上界。需R025补充连续嵌入oracle。Output: experiments/analysis/g1_oracle_decoder/ |
| R025 | G11 | G11 | Oracle NN decoder: upper-bound BLEU on continuous embeddings | Nearest-neighbor retrieval by encode_continuous() cosine similarity; measures mT5 input ceiling | CSL-Daily | train→test | oracle BLEU4 | MUST | DONE | conv1d_continuous oracle BLEU4=1.890, stgcn_continuous=1.652; mean NN cosine=0.87 (高度密集！). oracle ≈ mT5 baseline 1.742 → mT5已接近理论上界；G11 新发现：between-sequence语义密度是瓶颈 |
| R026 | G11 | G11 | Data scale hypothesis: mT5 learning curve | mT5 fine-tune at 1k/5k/10k/18k samples; measure BLEU4 vs data size | CSL-Daily | val | BLEU4 curves | MUST | DONE | 1k=0.213, 5k=0.747, 10k=0.761(plateau!), 18k=1.742. 5k→10k平台，10k→18k跳跃×2.3；<18k时BLEU_2/3全零。H4部分成立但oracle上界=1.89，mT5已近上界 |
| R027 | G-Phase | G (Plan A) | Semantic VAE encoder fine-tuning: contrastive + reconstruction dual loss | Conv1d VAE encoder fine-tune: freeze decoder/quantizer, train encoder+quantize_in; L=λ_recon×SmoothL1 + λ_contra×InfoNCE(sign↔text) | CSL-Daily | train | oracle BLEU4, mean NN cosine (R025-style) | MUST | TODO | Script: scripts/train_semantic_vae_finetune.py; Success gate: oracle BLEU4 > 3.0 AND mean NN cosine < 0.78 |
| R027-oracle | G-Phase | G (Plan A) | Oracle re-run on R027 encoder: measure embedding ceiling | R025-style NN retrieval BLEU4 using R027 fine-tuned encoder embeddings | CSL-Daily | train→test | oracle BLEU4, mean NN cosine | MUST | TODO | Run g2_oracle_continuous.py --vae_ckpt R027_checkpoint after R027 completes |
| R028 | G-Phase | G (Plan A) | mT5 fine-tune on R027 semantic embeddings | mT5-base LoRA fine-tune using R027 VAE encoder (same LoRA config as R006) | CSL-Daily | val/test | BLEU4 | MUST | TODO | Run only if R027-oracle gate passes (oracle BLEU4 > 3.0) |
